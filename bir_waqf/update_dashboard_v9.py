...
setInterval(function() {
    ...
    $('.modal-dialog, .modal').each(function() {
        var $m = $(this);
        var title = $m.find('.modal-title').text() || '';
        if (title.indexOf('جلب المعاملات') !== -1) {
            var $proj_field = $m.find('[data-fieldname="projects"]');
            if ($proj_field.length && !$proj_field.find('#btn-fetch-all-linked-projects-dialog').length) {
                var btn_html = `
                    <div style="margin-top: 10px; margin-bottom: 8px;" id="wrapper-btn-fetch-projects">
                    ...
                    <div style="font-size: 11px; color: #64748b; margin-top: 4px;">انقر هنا لاختيار كافة المشاريع ذات المعاملات في هذه الدفعة والمصرف تلقائياً</div>
                `;
                var $help = $proj_field.find('.help-box');
                if ($help.length) {
                    $help.html(btn_html);
                } else {
...
if (typeof $ !== 'undefined') {
    $(document).off('click.fetch_projs_global').on('click.fetch_projs_global', '#btn-fetch-all-linked-projects-dialog', function(e) {
        e.preventDefault();
        e.stopPropagation();
        ...
        if (!batch) {
            var $b_inp = $('[data-fieldname="import_batch"] input');
            if ($b_inp.length) batch = $b_inp.val();
        }

        if (!bank) {
            var $bk_inp = $('[data-fieldname="bank"] input');
            if ($bk_inp.length) bank = $bk_inp.val();
        }
        ...
        frappe.call({
            ...
            callback: function(r) {
                if (r.message && r.message.length > 0) {
                    ...
                    var $p_inp = $('[data-fieldname="projects"] input');
                    if ($p_inp.length) {
                        $p_inp.val(proj_str).trigger('change');
                    }
                    frappe.show_alert({
